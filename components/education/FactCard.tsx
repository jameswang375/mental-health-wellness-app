...
export default function FactCard({ fact }: { fact: Fact }) {
  ...
  return (
    <div className="bg-gray-900 rounded-xl border border-gray-800 shadow-sm p-5 flex flex-col gap-3">
      <p className="text-white font-medium leading-snug">{fact.statement}</p>

      {expanded && (
        <p className="text-sm text-gray-400 leading-relaxed">{fact.explanation}</p>
      )}
